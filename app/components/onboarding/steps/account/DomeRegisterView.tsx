...
export default function DomeRegisterView({
  ...
  const passwordError = touched.password && !passwordValid ? t('onboarding.password_min_length') : undefined;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Field className="gap-1.5" data-invalid={Boolean(emailError)}><FieldLabel htmlFor="account-email" className="text-xs">{t('onboarding.account_email_label')}</FieldLabel><div className="relative min-w-0"><span className="pointer-events-none absolute top-1/2 left-3 z-10 flex -translate-y-1/2 items-center text-muted-foreground" aria-hidden><Mail className="size-4" /></span><Input id="account-email" className="pl-9" type="text" inputMode="email" autoComplete="email" placeholder={t('onboarding.email_placeholder')} value={email} onChange={(e) => onEmailChange(e.target.value)} onBlur={onEmailBlur} disabled={isSubmitting} aria-invalid={Boolean(emailError) || undefined} /></div><FieldError className="text-xs">{emailError}</FieldError></Field>

      <Field className="gap-1.5" data-invalid={Boolean(passwordError)}><FieldLabel htmlFor="account-password" className="text-xs">{t('onboarding.account_password_label')}</FieldLabel><div className="relative min-w-0"><span className="pointer-events-none absolute top-1/2 left-3 z-10 flex -translate-y-1/2 items-center text-muted-foreground" aria-hidden><Lock className="size-4" /></span><Input id="account-password" className="pl-9" type="password" autoComplete="new-password" placeholder="••••••••" value={password} onChange={(e) => onPasswordChange(e.target.value)} onBlur={onPasswordBlur} disabled={isSubmitting} aria-invalid={Boolean(passwordError) || undefined} /></div><FieldError className="text-xs">{passwordError}</FieldError></Field>
