...
function ContextMenu({ state, onClose, onRename, onMove, onColorChange, onDelete, onNewFolder }: ContextMenuProps) {
  ...
  const itemStyle: React.CSSProperties = {
    ...
    textAlign: 'left', padding: '6px 12px', fontSize: 12.5,
  ...
  return (
    ...
    >
      <div style={{ padding: '4px 0' }}>
        ...
        >
          <Edit3 className="w-3.5 h-3.5 shrink-0" /><span>Renombrar</span>
        </button>
        ...
        >
          <FolderInput className="w-3.5 h-3.5 shrink-0" /><span>Mover a carpeta</span>
        </button>
        ...
        {isFolder && (
          ...
          >
            <FolderPlus className="w-3.5 h-3.5 shrink-0" /><span>Nueva subcarpeta</span>
          </button>
        )}
        ...
        {isFolder && (
          <>
            <button
              style={{ ...itemStyle, background: showColors ? 'var(--dome-bg-hover)' : 'transparent' }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = 'var(--dome-bg-hover)'; }}
              onMouseLeave={(e) => { if (!showColors) (e.currentTarget as HTMLButtonElement).style.background = 'transparent'; }}
              onClick={() => setShowColors((s) => !s)}
            >
              <span className="w-3.5 h-3.5 rounded-full shrink-0 border" style={{ background: currentColor?.startsWith('#') ? currentColor : 'var(--dome-accent)', borderColor: 'var(--dome-border)' }} />
              <span className="flex-1">Color</span>
              <ChevronDown className="w-3 h-3 shrink-0 opacity-50" style={{ transform: showColors ? 'rotate(0deg)' : 'rotate(-90deg)', transition: 'transform 150ms' }} />
            </button>
            {showColors && (
              <div className="flex flex-wrap gap-1.5 px-3 pb-2">
                {FOLDER_COLOR_OPTIONS.map((opt) => (
                  <button
                    ...
                    style={{ background: opt.value, outline: currentColor === opt.value ? `2px solid ${opt.value}` : 'none', outlineOffset: 2, border: 'none', cursor: 'pointer' }}
                  >
        ...
        <button
          style={{ ...itemStyle, color: 'var(--dome-error, #ef4444)' }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = 'var(--dome-bg-hover)'; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = 'transparent'; }}
          onClick={() => { onDelete(r); onClose(); }}
        >
          <Trash2 className="w-3.5 h-3.5 shrink-0" /><span>Eliminar</span>
        </button>
      </div>
    </div>
  );
      ...
      >
        ...
        {hovered && !isRenaming && node.resource && (
          <button
            ...
            style={{ width: 18, height: 18, background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--dome-text-muted)' }}
            title="Opciones"
