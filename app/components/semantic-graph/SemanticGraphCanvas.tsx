...
function installSemanticGraphSimulation(
  svgEl: SVGSVGElement,
  nodes: GraphNodeDatum[],
  edges: GraphEdgeDatum[],
  onEdgeClickRef: MutableRefObject<
    ((edge: GraphEdgeDatum, clientX: number, clientY: number) => void) | undefined
  >,
  onNodeDblRef: MutableRefObject<((node: GraphNodeDatum) => void) | undefined>,
): () => void {
  ...
  return function cleanupSemanticGraphSimulation() {
    link.on('click', null);
...
export default function SemanticGraphCanvas({
  ...
  useEffect(() => {
    ...
    return installSemanticGraphSimulation(svgEl, nodes, edges, onEdgeClickRef, onNodeDblRef);
  }, [nodes, edges]);

  return (
    <svg
      ref={svgRef}
      className="size-full min-h-[420px] touch-none select-none semantic-graph-svg"
      role="img"
