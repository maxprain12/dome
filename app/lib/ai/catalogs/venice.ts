/**
 * Venice Models Catalog
 * 
 * Privacy-focused models available through Venice AI.
 * Migrated from clawdbot/src/agents/venice-models.ts
 */
...
/** Venice uses credit-based pricing, costs vary by model */
export const VENICE_DEFAULT_COST: ModelCost = {
  input: 0,
  output: 0,
  cacheRead: 0,
  cacheWrite: 0,
 ...
 */
export type VenicePrivacyMode = 'private' | 'anonymized';

// =============================================================================
// Model Catalog Entry Type
// =============================================================================

interface VeniceCatalogEntry {
  id: string;
  name: string;
  reasoning: boolean;
...
export const VENICE_MODEL_CATALOG: readonly VeniceCatalogEntry[] = [
  // ============================================
 ...
 */
export function buildVeniceModelDefinition(entry: VeniceCatalogEntry): ModelDefinition & { privacy: VenicePrivacyMode } {
  return {
    id: entry.id,
    name: entry.name,
...
/**
 * Get all Venice models as ModelDefinition[]
 */
export function getVeniceModels(): ModelDefinition[] {
  return VENICE_MODEL_CATALOG.map(buildVeniceModelDefinition);
}

/**
 * Find a Venice model by ID
 */
export function findVeniceModel(modelId: string): ModelDefinition | undefined {
  const entry = VENICE_MODEL_CATALOG.find(m => m.id === modelId);
  return entry ? buildVeniceModelDefinition(entry) : undefined;
}

/**
 * Get the default Venice model
 */
export function getDefaultVeniceModel(): ModelDefinition {
  const entry = VENICE_MODEL_CATALOG.find(m => m.id === VENICE_DEFAULT_MODEL_ID);
  if (!entry) {
    throw new Error(`Default Venice model not found: ${VENICE_DEFAULT_MODEL_ID}`);
  }
  return buildVeniceModelDefinition(entry);
}

/**
 * Get all private (fully anonymous) Venice models
 */
export function getVenicePrivateModels(): ModelDefinition[] {
  return VENICE_MODEL_CATALOG
    .filter(m => m.privacy === 'private')
    .map(buildVeniceModelDefinition);
}

/**
 * Get all anonymized (proxied) Venice models
 */
export function getVeniceAnonymizedModels(): ModelDefinition[] {
  return VENICE_MODEL_CATALOG
    .filter(m => m.privacy === 'anonymized')
    .map(buildVeniceModelDefinition);
}

/**
 * Get all reasoning-capable Venice models
 */
export function getVeniceReasoningModels(): ModelDefinition[] {
  return VENICE_MODEL_CATALOG
...
/**
 * Get all vision-capable Venice models
 */
export function getVeniceVisionModels(): ModelDefinition[] {
  return VENICE_MODEL_CATALOG
 ...
 */
export async function discoverVeniceModels(): Promise<ModelDefinition[]> {
  try {
    ...
    for (const apiModel of data.data) {
      ...
      if (catalogEntry) {
        ...
        models.push({
          ...
          api: 'openai-completions',
          compat: {
            supportsTools: apiModel.model_spec.capabilities.supportsFunctionCalling,
            supportsStreaming: true,
