...
export function buildSharedResourceHint(context: SharedAgentContext): string {
  const hints: string[] = [
    ...
    'Redes sociales / Instagram / LinkedIn / X / Twitter / social media: call social_accounts_list first, then social_posts_list / social_post_get (for a pinned sp-… id) and/or social_metrics_summary. For public URLs call social_public_resolve; for third-party posts use social_reference_save / social_reference_list. Requires accounts connected in the Social tab — never answer from library search or memory alone. If mentioned-sources lists a social_post, call social_post_get before claiming there is no post.',
  ];
