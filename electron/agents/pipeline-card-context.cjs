...
function buildDeliverableInstructions(deliverable) {
  if (deliverable === 'artifact') {
    return [
      ...
      '',
      'Un informe solo en el chat o un bloque ```artifact:...``` inline **no** cumple este entregable.',
      'Debes crear una miniapp persistida en la biblioteca de Dome con herramientas:',
      '',
      '1. Organiza los hallazgos en una experiencia interactiva: KPIs, filtros, detalles y evidencia verificable.',
      '2. `artifact_create` con `{ artifact_type: "custom", title, html, css, data }`: implementa los controles y guarda los cambios con __dome_updateState. Usa document solo si el usuario pide un documento estático.',
      '3. En tu respuesta final indica el título del artefacto creado (resumen breve; el contenido completo vive en el artefacto).',
      '',
      'Prohibido en esta fase:',
      '- NO uses la herramienta `task` ni subagentes; invoca `artifact_create` directamente.',
