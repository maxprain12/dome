...
function buildDeliverableInstructions(deliverable) {
  if (deliverable === 'artifact') {
    return [
      ...
      'Debes crear un documento persistido en la biblioteca de Dome con herramientas:',
      '',
      '1. Prepara el informe completo en Markdown.',
      '2. `artifact_create` con `{ artifact_type: "document", title, content }` donde content es el Markdown.',
