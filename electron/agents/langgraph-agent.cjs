...
const { measurePrompt } = require('../prompts/prompt-budget.cjs');
...
const { summarizeToolResultForUi, formatToolResultForModel } = require('../tools/tool-result-format.cjs');
const {
  normalizeToolInput,
  getMissingRequiredFields,
  formatToolValidationError,
} = require('../tools/tool-input-normalize.cjs');
const { capLangChainTools, sanitizeLeakedToolManifestText } = require('../tools/tool-cap.cjs');
...
async function emitUsageFromCheckpoint(agent, config, onChunk, partial = true) {
  try {
    ...
    // `cumulative: true` marks this as a full snapshot of the thread's usage
    // (not an incremental delta), so consumers REPLACE rather than sum to avoid
    // double counting against the per-chunk partials.
    if (usage && onChunk) onChunk({ type: 'usage', usage, partial, cumulative: true });
  } catch {
    /* ignore */
  }
}
...
async function completeAgentRunAfterStream({
  ...
  if (aggregatedUsage && onChunk) {
    // Canonical final snapshot of the whole thread's usage (see emitUsageFromCheckpoint).
    onChunk({ type: 'usage', usage: aggregatedUsage, partial: false, cumulative: true });
  }
...
async function toLangChainMessages(messages, opts = {}) {
  const { normalizeUserMessage } = require('../ai/message-multimodal.cjs');
