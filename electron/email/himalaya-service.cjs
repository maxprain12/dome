...
function writeConfig() {
  ...
  const body = ['# Generated by Dome ù do not edit by hand.', '', ...rows.map(tomlAccountSection)].join('\n');
...
async function listEnvelopes(
  accountId,
  { folder = 'INBOX', page = 1, pageSize = 30, projectId = null, source = 'auto' } = {},
) {
  const id = resolveAccountId(accountId, projectId);
  ...
  if (source === 'cache' || source === 'auto') {
    ...
    if (source === 'cache' || cached.length > 0) {
      return {
        success: true,
        envelopes: cached,
        accountId: id,
...
/** Parse the header block from `message read -H From -H Subject ù`. */
 ...
 * `message export` writes `index.html` and `plain.txt` ù we prefer the HTML file.
 */
async function readMessage(accountId, messageId, { folder = 'INBOX', projectId = null, forceLive = false } = {}) {
  ...
  const emailStore = require('./email-store.cjs');
  if (!forceLive) {
    const cached = emailStore.getCachedMessage(id, messageId, folder);
  ...
  try {
    emailStore.upsertFolder(id, folder);
    const folderRow = emailStore.getFolderByRemote(id, folder);
    if (folderRow) {
      emailStore.upsertEnvelope(id, folderRow.id, { id: messageId });
      emailStore.cacheMessageBody(id, messageId, folder, { text: plain, html });
    }
  } catch (err) {
