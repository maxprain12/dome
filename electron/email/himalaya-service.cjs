...
function cacheLooksBroken(envelopes) {
  if (!Array.isArray(envelopes) || envelopes.length === 0) return false;
  // All rows missing subject AND from ? leftover from the wipe-on-read bug.
  return envelopes.every((e) => !e?.subject && !e?.from);
}

async function listEnvelopes(
  accountId,
  { folder = 'INBOX', page = 1, pageSize = 200, projectId = null, source = 'auto' } = {},
) {
  const id = resolveAccountId(accountId, projectId);
  ...
  if (source === 'cache' || source === 'auto') {
    const cached = emailStore.listCachedEnvelopes(id, folder, { limit: pageSize, offset });
    const useCache =
      source === 'cache' || (cached.length > 0 && !(source === 'auto' && cacheLooksBroken(cached)));
  ...
  try {
    ...
    console.warn('[himalaya] cache live envelopes failed:', err?.message || err);
  }
  return { success: true, envelopes, accountId: id, folder, source: 'live' };
}

async function searchEnvelopes(accountId, query, { folder = 'INBOX', pageSize = 200, projectId = null } = {}) {
  const id = resolveAccountId(accountId, projectId);
  if (!id) return { success: false, error: 'No email account configured', envelopes: [] };
  const emailStore = require('./email-store.cjs');
...
async function readMessage(accountId, messageId, { folder = 'INBOX', projectId = null, forceLive = false } = {}) {
  ...
  if (!forceLive) {
    const cachedRow = resolved.row;
    if (cachedRow?.body_html || cachedRow?.body_text) {
      return {
        success: true,
        message: {
          html: cachedRow.body_html,
          plain: cachedRow.body_text,
          text: cachedRow.body_text,
        },
        accountId: id,
        messageId: uid,
    ...
    const cached = emailStore.getCachedMessage(id, uid, resolvedFolder);
    if (cached?.body_html || cached?.body_text) {
      return {
        success: true,
        message: {
          html: cached.body_html,
          plain: cached.body_text,
  ...
  try {
    await runHimalaya(['message', 'export', String(uid), '-f', resolvedFolder, '-d', dest], { accountId: id });
  ...
  if (!html && !plain) {
    const data = await runHimalaya(
      ['message', 'read', String(uid), '-f', resolvedFolder, '--no-headers', '--preview'],
      { accountId: id },
    );
  ...
  try {
    // Only cache the body ? never upsert a bare { id } envelope here.
    // That used to wipe subject/from in SQLite (ON CONFLICT overwrote with nulls).
    emailStore.cacheMessageBody(id, uid, resolvedFolder, { text: plain, html });
  } catch (err) {
    console.warn('[himalaya] cache body failed:', err.message);
  }
