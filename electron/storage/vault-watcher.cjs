...
async function ensureFolderChain(projectId, folderSegs, deps) {
  ...
  for (const seg of folderSegs) {
    ...
    if (!folder) {
      ...
      await db.run(
        ...
        [fid, projectId, 'folder', seg, null, null, parentId, null, now, now],
      );
...
async function enrichImportedFile(id, absPath, type, mime, deps) {
  try {
    ...
    try {
      ...
      if (thumb) await db.run('UPDATE resources SET thumbnail_data = ? WHERE id = ?', [thumb, id]);
    } catch { /* non-fatal */ }
    let text = null;
    try {
    ...
    if (text) {
      await db.run('UPDATE resources SET content = ?, content_text = ? WHERE id = ?', [text, text, id]);
    }
...
async function importExternalNote(raw, ctx, deps) {
  const { database, semanticIndexScheduler, windowManager } = deps;
  const db = database.getDB();
  const segments = ctx.relPath.split('/').filter(Boolean);
  if (segments.length === 0) return;
  const folderId = await ensureFolderChain(ctx.projectId, segments.slice(0, -1), deps);

  let noteId = vaultStore.parseFrontmatterId(raw);
  if (!noteId || !/^[\w-]+$/.test(noteId) || (await db.get('SELECT id FROM resources WHERE id=?', [noteId]))) {
  ...
  await db.run(
    ...
    [noteId, ctx.projectId, 'note', title, null, null, folderId, ctx.relPath, text, hash, null, now, now],
  );
...
async function importExternalBinary(absPath, buf, ext, ctx, deps) {
  ...
  await db.run(
    ...
    [id, ctx.projectId, type, filename, null, null, null, mime, buf.length, hash, filename, folderId, ctx.relPath, hash, null, now, now],
  );
  windowManager.broadcast('resource:created', {
...
async function handleChange(absPath, deps) {
  const { database, semanticIndexScheduler, windowManager } = deps;
  ...
  let row = await db.get('SELECT id, type, content_hash, vault_path FROM resources WHERE project_id = ? AND vault_path = ?', [ctx.projectId, ctx.relPath]);
  if (!row && isMd) {
    const fid = vaultStore.parseFrontmatterId(buf.toString('utf8'));
    if (fid) {
      const byId = await db.get("SELECT id, type, content_hash, vault_path FROM resources WHERE id = ? AND type = 'note'", [fid]);
      if (byId) row = byId;
    }
  }
  if (!row) {
    if (isMd) await importExternalNote(buf.toString('utf8'), ctx, deps);
    else await importExternalBinary(absPath, buf, ext, ctx, deps);
    return;
  }

  if (row.content_hash === hash) {
    if (row.vault_path !== ctx.relPath) await db.run('UPDATE resources SET vault_path = ? WHERE id = ?', [ctx.relPath, row.id]);
    return;
  }
  ...
  if (isMd) {
    ...
    await db.run('UPDATE resources SET vault_path = ?, content_text = ?, content_hash = ?, updated_at = ? WHERE id = ?',
      [ctx.relPath, text, hash, now, row.id]);
  } else {
    await db.run('UPDATE resources SET vault_path = ?, content_hash = ?, file_size = ?, updated_at = ? WHERE id = ?',
      [ctx.relPath, hash, buf.length, now, row.id]);
...
function handleUnlink(absPath, deps) {
  ...
  _pendingUnlinks.set(absPath, setTimeout(async () => {
    ...
    try {
      ...
      await database.getQueries().deleteResource.run(row.id);
...
async function scanRoot(rootAbs, deps) {
  const db = deps.database.getDB();
  const stack = [rootAbs];
  while (stack.length) {
    const dir = stack.pop();
    ...
    for (const e of entries) {
      ...
      if (ext === '.md') {
        ...
        if (fid && (await db.get('SELECT id FROM resources WHERE id=?', [fid]))) continue;
        try { await importExternalNote(raw, ctx, deps); } catch (err) { console.warn('[VaultWatcher] scan note import failed:', err.message); }
      } else {
        try { await importExternalBinary(abs, buf, ext, ctx, deps); } catch (err) { console.warn('[VaultWatcher] scan file import failed:', err.message); }
      }
    }
  }
}
...
function start(deps) {
  ...
  _watcher
    .on('add', async (p) => { await handleChange(p, deps); })
    .on('change', async (p) => { await handleChange(p, deps); })
    .on('addDir', async (p) => { await handleAddDir(p, deps); })
  ...
  setTimeout(async () => {
    try { for (const t of targets) await scanRoot(t, deps); } catch (err) { console.warn('[VaultWatcher] scan failed:', err.message); }
  }, 4000);
}
...
function addRoot(absPath) {
  try {
    if (_watcher && absPath) {
      ...
      if (_deps) setTimeout(async () => { try { await scanRoot(absPath, _deps); } catch { /* */ } }, 1000);
    }
