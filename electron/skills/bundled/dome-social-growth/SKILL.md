...
- Soft campaigns: propose a campaign name + goal, then draft posts tagged with that `campaign` string (no separate campaigns table).
- Prefer `social_accounts_list` / `social_posts_list` / `social_metrics_summary` before inventing KPIs; use `social_post_draft` to persist drafts (publish is HITL).
...
- Do not invent a campaigns schema — use the post `campaign` field.
