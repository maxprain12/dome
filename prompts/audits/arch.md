...
1. Ejecuta `pnpm run depcruise` y **no** propongas silenciar reglas salvo con ADR.
...
- `pnpm run typecheck` / `pnpm run lint` / `pnpm run depcruise`
- `pnpm run check:ipc-inventory` si el cambio toca canales
