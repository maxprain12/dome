...
> **Context:** `prompts/shared/project-context.md` (v5), `AGENTS.md` (§ Baseline 2026-04). Lockfile: `pnpm-lock.yaml`; CI: `pnpm install --frozen-lockfile`.
...
`pnpm add <pkg>@<patched-version> --ignore-scripts`
...
```bash
pnpm outdated --json > /tmp/pnpm-outdated.json
```

For every package where `current < wanted` (i.e. the semver range allows it):

- Bump to `wanted` via: `pnpm add <pkg>@<wanted> --ignore-scripts`
...
The project uses `pnpm` for CI/CD. After any `package.json` change, regenerate
`pnpm-lock.yaml` so it agrees with the manifest:

```bash
rm -f pnpm-lock.yaml
pnpm install --ignore-scripts      # regenerates pnpm-lock.yaml
```

Never commit a stale `pnpm-lock.yaml` — CI runs `pnpm install --frozen-lockfile` which aborts on
any drift between `package.json` and the lockfile. The project is **pnpm**-only.
